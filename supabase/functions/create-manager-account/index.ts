...
serve(async (req) => {
  ...
  const { data: role, error: roleError } = await admin
    .from('user_roles').select('role').eq('user_id', requesterData.user.id).single();
  if (roleError || role?.role !== 'admin') {
    return json({ error: 'Only admin users can create manager accounts' }, 403);
  }

  const { email, password, managerId } = await req.json().catch(() => ({}));
  ...
  if (!managerId) return json({ error: 'Missing manager id' }, 400);

  const { data: manager, error: mgrError } = await admin
    .from('managers').select('id,name').eq('id', managerId).single();
  if (mgrError || !manager) return json({ error: 'Manager not found' }, 404);

  const meta = { role: 'manager', manager_id: managerId, must_change_password: true };
  ...
  if (authUser) {
    ...
    const { data: created, error: createError } = await admin.auth.admin.createUser({
      email: normalizedEmail, password, email_confirm: true, user_metadata: meta,
    });
  ...
  const { data: existingRole } = await admin
    .from('user_roles').select('user_id').eq('user_id', userId).maybeSingle();

  const rolePayload = { user_id: userId, role: 'manager', manager_id: managerId, rep_id: null, supervisor_id: null };
  ...
  return json({
    userId, email: normalizedEmail, managerId, managerName: manager.name,
